  ...
  const handleSubmit = async (e) => {
    ...
    try {
      const payload = { 
        ...
        shop_id: shopId // ✅ HIFADHI CHINI YA DUKA, SI MTU
      };
  ...
  return (
    <div style={{ background: colors.surface, padding: '24px', borderRadius: '12px', border: `1px solid ${colors.border}` }}>
      ...
      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '24px', flexWrap: 'wrap', gap: '16px' }}>
        <input type="text" placeholder={t.searchPlaceholder} value={search} onChange={e => setSearch(e.target.value)} style={{ flex: '1 1 300px', padding: '12px', background: colors.bg, color: colors.text, border: `1px solid ${colors.border}`, borderRadius: '8px' }} />
        <button onClick={() => { setShowForm(true); setEditing(null); setForm({ name: '', cost_price: '', selling_price: '', stock_quantity: '', barcode: '', category: '' }); }} style={{ padding: '12px 20px', background: THEME.colors.primary, color: '#fff', border: 'none', borderRadius: '8px', cursor: 'pointer', fontWeight: 'bold' }}>{t.addProduct}</button>
      </div>
      ...
      {showForm && (
        <form onSubmit={handleSubmit} style={{ background: isDark ? '#1e293b' : '#f8fafc', padding: '24px', borderRadius: '12px', marginBottom: '24px', display: 'grid', gap: '16px', gridTemplateColumns: 'repeat(auto-fit, minmax(200px, 1fr))' }}>
          ...
          <div style={{ gridColumn: '1 / -1', display: 'flex', gap: '16px' }}>
            <button type="submit" disabled={loading} style={{ flex: 1, padding: '12px', background: loading ? '#64748b' : THEME.colors.success, color: '#fff', border: 'none', borderRadius: '8px', cursor: 'pointer', fontWeight: 'bold' }}>{loading ? g.loading : t.saveProduct}</button>
            <button type="button" onClick={() => setShowForm(false)} style={{ flex: 1, padding: '12px', background: colors.surface, color: colors.text, border: `1px solid ${colors.border}`, borderRadius: '8px', cursor: 'pointer', fontWeight: 'bold' }}>{g.cancel}</button>
          </div>
        </form>
      )}
      ...
      {loading ? <p style={{ textAlign: 'center', color: colors.textSec }}>{g.loading}</p> : filtered.length === 0 ? <p style={{ textAlign: 'center', color: colors.textSec }}>{t.noProducts}</p> : (
        <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fill, minmax(250px, 1fr))', gap: '16px' }}>
          {filtered.map(p => {
            ...
            return (
              <div key={p.id} style={{ border: `1px solid ${colors.border}`, borderRadius: '12px', padding: '20px', background: isDark ? '#1e293b' : '#ffffff' }}>
                <h4 style={{ margin: '0 0 8px', fontSize: '16px', color: colors.text }}>{p.name}</h4>
                <p style={{ margin: '0 0 12px', color: THEME.colors.success, fontWeight: 'bold', fontSize: '1.2rem' }}>{fmt(p.selling_price)} TSh</p>
                
                {/* ✅ FICHIA FAIDA NA BEI YA KUNUNUA KAMA SI ADMIN */}
                {userRole === 'admin' && (
                  <div style={{ marginTop: '8px', paddingTop: '8px', borderTop: `1px dashed ${colors.border}` }}>
                    <p style={{ margin: '4px 0', fontSize: '13px', color: THEME.colors.warning }}>Kununua: {fmt(p.cost_price)} TSh</p>
                    <p style={{ margin: '4px 0', fontSize: '13px', color: THEME.colors.success, fontWeight: 'bold' }}>📈 Faida: {fmt(profit)} TSh</p>
                  </div>
                )}
                
                <p style={{ margin: '8px 0 0', fontSize: '13px', color: colors.textSec }}>{t.stock}: {p.stock_quantity ?? '-'}</p>
                <div style={{ display: 'flex', gap: '8px', marginTop: '16px' }}>
                  <button onClick={() => handleEdit(p)} style={{ flex: 1, padding: '8px', background: THEME.colors.primary, color: '#fff', border: 'none', borderRadius: '6px', cursor: 'pointer', fontSize: '13px' }}>{t.editProduct}</button>
                  <button onClick={() => handleDelete(p.id)} style={{ flex: 1, padding: '8px', background: THEME.colors.error, color: '#fff', border: 'none', borderRadius: '6px', cursor: 'pointer', fontSize: '13px' }}>{t.deleteProduct}</button>
                </div>
              </div>
            );
          })}
